...
void RenderWindow::renderText()
{
    ...
    TTF_Font* Sans = TTF_OpenFont("font/times-new-roman.ttf", 24);
    ...
    SDL_Surface* surfaceMessage =
        TTF_RenderText_Solid(Sans, "LyRs kalandjai", White); 

    // now you can convert it into a texture
    SDL_Texture* Message = SDL_CreateTextureFromSurface(renderer, surfaceMessage);

    SDL_Rect Message_rect; //create a rect
    Message_rect.x = 489;  //controls the rect's x coordinate 
    Message_rect.y = 85; // controls the rect's y coordinte
